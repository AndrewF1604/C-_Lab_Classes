...
namespace Lab3_homework
{
    ...
    partial class TestSubmarine
    {
        ...
        // unique means the specified constructor should be the only one in the class
        private static bool TestConstructor(string className, List<string> arguments, bool unique = true)
        {
            testNumber++;
            Type type = Type.GetType(nms + "." + className);
            ...
            foreach (var constructor in constructors)
            {
                ...
                if (param.Length == arguments.Count)
                {
                    ...
                    if (test)
                    {
                        ...
                        if(unique && constructors.Length > 1)
                        {
                            testNumber++;
                            failed++;
                            Console.ForegroundColor = ConsoleColor.Red;
                            Console.WriteLine("Test " + testNumber + " failed: class " + className + " should have only one constructor.");
                            return false;
                        }
                        return true;
                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (param.Length == arguments.Count)
                    {
                        for (int i = 0; i < arguments.Count; i++)
                        {
                            if (!param[i].ParameterType.ToString().EndsWith("." + arguments[i])) ans = false;
                        }
                    }
                    else ans = false;
                }
        ...
        private static bool TestWeightVolume(string className, object[] parameters, object inst = null)
        {
            ...
            if (oldVolume != null && oldWeight != null) type.GetProperty("Volume").SetValue(instance, oldVolume + 19.0);
            ...
            testNumber++;
            oldVolume = (double?)type.GetProperty("Volume").GetValue(instance);
            oldWeight = (double?)type.GetProperty("Weight").GetValue(instance);
            if (oldVolume != null && oldWeight != null) type.GetProperty("Weight").SetValue(instance, oldWeight + 19.0);
